#DON'T EDIT. AUTOGENERATED FILE
{buildCratesLib, allCrates}:
rec {
  rusty-release_0_3_1 = buildCratesLib {
    name = "rusty-release";
    version = "0.3.1";
    hash = "6f80dee57bdd2eb126c4dda790700e9382dea7a80a78465fd9fbcc711db1b17c";
    deps = with allCrates; [  all__toml.toml_0_1 all__semver.semver_0_2 all__tempfile.tempfile_2_1 all__term.term_0_4 all__rustc-serialize.rustc-serialize_0_3 all__clap.clap_2_2 ];
  };
  rusty-release_0_3_2 = buildCratesLib {
    name = "rusty-release";
    version = "0.3.2";
    hash = "b21528deb0de48af7c2d0fd55ad380226d2640143e4a936263bcefefffb7fa87";
    deps = with allCrates; [  all__term.term_0_4 all__toml.toml_0_1 all__semver.semver_0_2 all__tempfile.tempfile_2_1 all__clap.clap_2_2 all__rustc-serialize.rustc-serialize_0_3 ];
  };
  rusty-release_0_3_3 = buildCratesLib {
    name = "rusty-release";
    version = "0.3.3";
    hash = "8fd3d56543340a35bb695afd5a57b9971ea7e7554ecb228eacfdac22d6aa179d";
    deps = with allCrates; [  all__toml.toml_0_1 all__term.term_0_4 all__clap.clap_2_2 all__tempfile.tempfile_2_1 all__semver.semver_0_2 all__rustc-serialize.rustc-serialize_0_3 ];
  };
  rusty-release_0_3_4 = buildCratesLib {
    name = "rusty-release";
    version = "0.3.4";
    hash = "d4814810845af9d347f856658911f6eae647f74a5ba2b1acbf6e4ae2277674f6";
    deps = with allCrates; [  all__semver.semver_0_2 all__term.term_0_4 all__clap.clap_2_2 all__tempfile.tempfile_2_1 all__toml.toml_0_1 all__rustc-serialize.rustc-serialize_0_3 ];
  };
  rusty-release_0_3_5 = buildCratesLib {
    name = "rusty-release";
    version = "0.3.5";
    hash = "48f0ae094516b6182b437394816f9348ba97513c98bd8244ac7a1daf9a740d21";
    deps = with allCrates; [  all__rustc-serialize.rustc-serialize_0_3 all__clap.clap_2_2 all__semver.semver_0_2 all__term.term_0_4 all__tempfile.tempfile_2_1 all__toml.toml_0_1 ];
  };
  rusty-release_0_3_6 = buildCratesLib {
    name = "rusty-release";
    version = "0.3.6";
    hash = "2667a8ccebfdc3a9ce1f1228b495e8454646512070d6c7cc1f653885ca12fb06";
    deps = with allCrates; [  all__tempfile.tempfile_2_1 all__toml.toml_0_1 all__rustc-serialize.rustc-serialize_0_3 all__semver.semver_0_2 all__clap.clap_2_2 all__term.term_0_4 ];
  };
  "rusty-release_0_3" = rusty-release_0_3_6;
  "rusty-release_0" = rusty-release_0_3_6;}