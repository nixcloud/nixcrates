...
let
  ...
  depsStringCalc = pkgs.lib.fold ( dep: str: "${str} --extern ${normalizeName dep.name}=${dep}/lib${normalizeName dep.name}.rlib") "";       #nameFix attempt
...

{
  ...
  nix-crates = stdenv.mkDerivation rec {
    ...
    buildPhase = ''
      ...
      ${rustc}/bin/rustc $src/main.rs --crate-type "bin" --emit=dep-info,link --crate-name main -L dependency=mylibs ${crates}
    '';
  };
}
